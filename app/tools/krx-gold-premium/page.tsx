...
// getExchangeRate moved to services/exchangeRate.ts
...
import GoldInfoSection from './GoldInfoSection'
...
export default async function KRXGoldPremiumPage() {
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: 'KRX 금 시세는 어떻게 결정되나요?',
        acceptedAnswer: {
          '@type': 'Answer',
          text: 'KRX 금시장의 시세는 주식 시장처럼 매수자와 매도자의 주문이 만나는 지점에서 실시간으로 결정됩니다. 국제 금값과 원/달러 환율의 영향을 가장 크게 받지만, 국내의 수급 상황(금 수요 등)에 따라 국제 가격과 괴리(프리미엄)가 발생할 수 있습니다.',
        },
      },
      {
        '@type': 'Question',
        name: '국제 금 시세와 가격이 다른 이유는 무엇인가요?',
        acceptedAnswer: {
          '@type': 'Answer',
          text: '이론적으로는 국제 금값(USD/oz) × 환율(KRW/USD)가 국내 금값이어야 하지만, 실제로는 국내 시장 참여자들의 심리, 수급 불균형, 거래 시간의 차이 등으로 인해 가격 차이가 발생합니다. 이 차이를 프리미엄이라고 부르며, 이를 확인하고 투자하는 것이 중요합니다.',
        },
      },
      {
        '@type': 'Question',
        name: '금 1돈은 몇 그램(g)인가요?',
        acceptedAnswer: {
          '@type': 'Answer',
          text: '금 1돈은 정확히 3.75g입니다. 국제 시장에서는 트로이온스(Troy Ounce, 약 31.1g) 단위를 주로 사용하며, KRX 금시장은 1g 단위로 거래됩니다.',
        },
      },
    ],
  }
