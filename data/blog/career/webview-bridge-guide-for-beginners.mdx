...
summary: '웹뷰 브릿지란 무엇인가? postMessage 기반의 웹뷰 브릿지 동작 원리, Handshake 프로토콜, 타이밍 이슈 해결법, 보안 모범 사례까지. 주니어 프론트엔드 개발자를 위한 웹뷰 브릿지 입문 가이드.'
...
`postMessage`가 널리 사용되기 전에는 **URL Scheme** 방식이 주로 사용되었습니다. 지금도 많은 프로젝트에서 볼 수 있는 패턴이죠:
...
| **비동기 응답 불가** | `await`로 결과를 기다릴 수 없음                                        |
...
타이밍 이슈는 **양방향 모두**에서 발생할 수 있습니다:

#### 케이스 1: 네이티브 → 웹 (더 까다로운 케이스)

네이티브 앱이 웹에 데이터를 전달하려 할 때 발생합니다. **실무에서 더 자주 문제가 되는 케이스**입니다.
...
**문제점**: `onPageFinished`(Android) 또는 `webViewDidFinishLoad`(iOS)는 **HTML 파싱 완료**를 의미하지, **JavaScript 초기화 완료**를 보장하지 않습니다.
...
- `undefined is not a function` 에러
- 간헐적으로만 발생 (재현 불가능)
...
웹이 네이티브보다 먼저 로드되면, 네이티브가 아직 브릿지를 설정하기 전에 웹이 호출을 시도합니다. 결과는 **영원히 응답을 기다리거나**, **에러 없이 조용히 실패**합니다.
...
위 문제들을 근본적으로 해결하는 방법이 바로 **Handshake**입니다.
...
<div className="-mx-2 flex flex-wrap overflow-hidden xl:-mx-2">
  <div className="my-1 w-full overflow-hidden px-2 xl:my-1 xl:w-full xl:px-2">
    <Image
      alt="Webview Handshake Sequence Diagram"
      src="/static/images/career/webview-handshake-diagram.png"
      width={1200}
      height={630}
      className="w-full rounded-lg object-cover"
    />
...
- [ ] 어떤 브릿지 라이브러리 사용? (예: `react-native-webview`, 또는 자체 구현)
...
- [Shopify의 웹뷰 네이티브화 전략 (Mobile Bridge)](https://shopify.engineering/mobilebridge-native-webviews)
