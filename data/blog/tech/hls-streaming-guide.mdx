...
1.  **마스터/미디어 플레이리스트 요청**: 스트림 정보를 파악하고 네트워크에 맞는 화질 선택.
2.  **키 요청**: 암호화된 콘텐츠를 풀기 위해 m3u8에 명시된 키서버로 인증 토큰과 함께 요청.
3.  **검증 및 발급**: 키서버는 토큰을 검증하고, 유효하면 AES-128 키(16바이트)를 반환.
4.  **다운로드 및 복호화**: 클라이언트는 이 키로 암호화된 `.ts` 조각을 실시간으로 복호화하여 재생.
...
1.  **조각난 구조**: 파일 하나가 아닌 수십, 수백 개의 조각이라 탈취가 번거로움.
2.  **키 보호**: 인증된 사용자에게만 키 발급, 토큰 검증, 세션 관리 등 추가 보안 적용 가능.
3.  **시간 제한 토큰**: URL에 유효 시간을 설정하여 링크 공유 방지.
...
hls.js는 브라우저 내부에서 **Transmuxing**(TS → MP4 실시간 변환)을 처리하여, 기본적으로 TS 재생을 지원하지 않는 브라우저에서도 재생이 가능하게 합니다.
...
- **원인 1: 헤더 인증 불가 (Header Auth Fail)**
  - **문제**: iOS(Safari)의 네이티브 플레이어는 개발자가 HTTP 요청 헤더를 조작할 수 없습니다 (`Authorization` 헤더 추가 불가).
  - **해결**: **보안 토큰을 쿼리 스트링(Query String)이나 쿠키(Cookie)**로 넘겨야 합니다.
  - **추천**: AWS CloudFront Signed URL 같은 **Presigned URL** 방식을 사용하여 URL 자체에 서명과 만료 시간을 포함시키세요.

- **원인 2: 코덱 및 스펙 불일치**
  - **증상**: 재생 버튼 눌러도 무반응이거나 검은 화면.
  - **체크**:
